import sys
import os
sys.path.append(os.path.join(os.path.dirname(__file__), 'j:\site-packages'))
import pygame
...
def main():
    ...
    # ブロックの作成(14*10)
    num_block = 15 if num_x == 400 else 22
    for x in range(1, num_block):
        for y in range(1, 11):
            Block("block.png", x, y)
    ...
    while (1):
        clock.tick(speed)      # フレームレート(60fps)
        screen.fill((0,20,0))
        # 全てのスプライトグループを更新
        group.update()
        # 全てのスプライトグループを描画
